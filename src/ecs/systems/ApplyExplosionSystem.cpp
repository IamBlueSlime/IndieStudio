        ...
        manager.template forEntitiesWith<IsBomb, Position, IsExploding, ExplosionRange>(
            [&manager](auto &data, [[gnu::unused]] auto id) {
                ...
                for (int i = 0; i < Range.explosionRangeLeft; i++) {
                    if (world->getPattern()->get(Position.x - i, 1, Position.z + i) == IndieStudio::MapPattern::TileType::BREAKABLE_BLOCK) {
                        world->getPattern()->set(Position.x - i, 1, Position.z + i, IndieStudio::MapPattern::TileType::BOMB_EXPLOSION);
